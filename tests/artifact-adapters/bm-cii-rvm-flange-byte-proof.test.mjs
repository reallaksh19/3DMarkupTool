...
assert.equal(state.rvmByteProof.writerBridge, 'composite-pipe-bend-flange-test-byte-proof.v1');
assert.deepEqual(state.rvmByteProof.writerBridges, ['src/artifact-adapters/rvm-code4-torus-test-byte-writer.js', 'src/artifact-adapters/rvm-flange-cylinder-test-byte-writer.js']);
assert.equal(state.rvmByteProof.artifactSegments.length, 2);
assert.equal(state.rvmByteProof.artifactSegments[0].segmentKind, 'pipe-bend');
assert.equal(state.rvmByteProof.artifactSegments[0].writerBridge, 'src/artifact-adapters/rvm-code4-torus-test-byte-writer.js');
assert.equal(state.rvmByteProof.artifactSegments[1].segmentKind, 'flange');
assert.equal(state.rvmByteProof.artifactSegments[1].writerBridge, 'src/artifact-adapters/rvm-flange-cylinder-test-byte-writer.js');
