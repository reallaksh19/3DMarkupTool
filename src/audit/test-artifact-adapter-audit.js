const TEST_ARTIFACT_ADAPTER_AUDIT_SCHEMA = 'TestArtifactAdapterAudit.v1';
const COUNT_KEYS = [
  'rvmArtifactByteLength',
  'attArtifactTextLength',
  'glbArtifactByteLength',
  'sourceTraceCount',
  ...
  'blockedArtifactItemCount',
  'deferredArtifactItemCount',
  'blockedFlangeCount',
  'blockedValveCount',
  'blockedBendCount',
...
const BOOLEAN_KEYS = [
  ...
  'attArtifactBlocked',
  'attArtifactChecksumPresent',
  'glbArtifactReady',
  'glbArtifactGenerated',
  'glbArtifactBlocked',
  'glbArtifactChecksumPresent',
  'runtimeTouched',
  'browserTouched',
  'canvasTouched',
  'ok'
...
export function assertTestArtifactAdapterAudit(audit, expectations = {}) {
  const errors = [];
  if (!audit || typeof audit !== 'object') errors.push('audit must be an object');
  if (audit?.schema !== TEST_ARTIFACT_ADAPTER_AUDIT_SCHEMA) errors.push(`schema must be ${TEST_ARTIFACT_ADAPTER_AUDIT_SCHEMA}`);
  ...
  if (errors.length) throw new Error(`Test artifact adapter audit invalid: ${errors.join('; ')}`);
  return { schema: 'TestArtifactAdapterAuditAssertion.v1', ok: true, errorCount: 0, errors: [] };
}
