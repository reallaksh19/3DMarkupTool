...
assert.match(bootstrap, /measure-polyline-viewpad-20260619/, 'safe bootstrap cache key must bump for the current viewpad tool pack');
...
assert.match(controller, /new THREE\.Box3\(\)\.setFromObject/, 'controller must derive section bounds from the selected object');
assert.match(controller, /expandByScalar\(sectionPadding\(box\)\)/, 'section box must include a small padding around selected bounds');
assert.match(controller, /new THREE\.Plane/g, 'section box must generate renderer clipping planes');
assert.match(controller, /renderer\.localClippingEnabled = true/, 'section box must enable local renderer clipping');
assert.match(controller, /renderer\.clippingPlanes = planes/, 'section box must assign renderer clipping planes');
assert.match(controller, /rt\.clippingMode = 'box'/, 'section box must set runtime clipping mode to box');
...
assert.match(controller, /renderOnce|viewer:request-render/, 'controller must request render after applying clipping');
