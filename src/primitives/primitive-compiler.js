...
export function compileResolvedGeometryToPrimitives(resolvedGeometry, geometryAudit, options = {}) {
  ...
  for (const frame of Array.isArray(resolvedGeometry?.itemFrames) ? resolvedGeometry.itemFrames : []) {
    items.push(itemRecord(frame.itemId, frame.geometryStatus === 'catalogueFrameResolved' ? 'deferred' : 'procedural', frame.sourceRef));
    ...
    if (frame.geometryStatus === 'resolved' && frame.resolver === 'straightPipeGeometry.v1') {
      ...
      if (!Number.isFinite(radiusMm)) {
        deferredPrimitives.push(deferred(frame, 'missing pipe diameter/radius evidence'));
        continue;
      }
      primitives.push(straightPipeCylinder(frame, radiusMm));
      continue;
    }
    if (frame.geometryStatus === 'catalogueFrameResolved') {
      deferredPrimitives.push(deferred(frame, 'component primitive compiler not implemented in Phase 6'));
    }
  }
  ...
  for (const support of Array.isArray(resolvedGeometry?.supportPlacements) ? resolvedGeometry.supportPlacements : []) {
    ...
    if (!canCompile) continue;
    deferredPrimitives.push({
      sourceItemId: support.itemId,
      family: 'support',
      type: support.supportFamily,
      geometryStatus: 'deferred',
      reason: 'support primitive compiler not implemented in Phase 6',
  ...
  for (const unresolved of Array.isArray(resolvedGeometry?.unresolvedGeometry) ? resolvedGeometry.unresolvedGeometry : []) {
    ...
    if (!canCompile) continue;
    blockedPrimitives.push({
      sourceItemId: unresolved.itemId,
      family: unresolved.family,
      type: unresolved.type,
      geometryStatus: 'blocked',
      reason: unresolved.reason || 'unresolved geometry',
      sourceRef: unresolved.sourceRef
    });
  }

  return {
    schema: PRIMITIVE_MODEL_SCHEMA,
    graphId,
    sourceGraphId: graphId,
    sourceGeometryId: resolvedGeometry?.id || resolvedGeometry?.graphId || graphId,
    units: resolvedGeometry?.units || options.units || 'mm',
    axisBasis: { ...(resolvedGeometry?.axisBasis || {}), authoring: resolvedGeometry?.axisBasis?.authoring || options.authoringBasis || 'canvas-current' },
    items,
    primitives,
    blockedPrimitives,
    deferredPrimitives,
...
export function buildPrimitiveCompilationAudit(resolvedGeometry, primitiveModel, geometryAudit, options = {}) {
  const validation = validateResolvedPrimitiveModelContract(primitiveModel, {
  ...
  const audit = {
    ...
    torusPrimitiveCount: primitives.filter((entry) => entry.primitiveKind === 'TORUS' || entry.kind === 'TORUS').length,
...
function itemRecord(id, resolutionMode, sourceRef) {
  return { id, sourceGraphItemId: id, resolutionMode, sourceRef };
}

function straightPipeCylinder(frame, radiusMm) {
  return {
    primitiveId: `PRIM-${frame.itemId}`,
    sourceItemId: frame.itemId,
    sourceRouteId: frame.routeId,
    primitiveKind: 'CYLINDER',
    primitiveCode: 8,
    center: [...frame.center],
    axis: [...frame.axis],
    lengthMm: Number(frame.lengthMm),
    radiusMm,
    basis: 'authoring',
...
function deferred(frame, reason) {
  return {
    sourceItemId: frame.itemId,
    family: frame.family,
    type: frame.type,
...
function deterministicRadius(frame) {
  const radius = Number(frame?.radiusMm);
  if (Number.isFinite(radius) && radius > 0) return radius;
  const diameter = Number(frame?.diameterMm);
