...
export const RVM_CHUNK_HIERARCHY_SCHEMA = 'RvmChunkHierarchyValidator.v1';
...
const ALLOWED_CHUNKS = new Set(['HEAD', 'MODL', 'CNTB', 'PRIM', 'CNTE', 'END:']);

/**
 * Validates the generated RVM chunk stream and its ATT hierarchy counterpart.
 * This intentionally checks only the generated conservative writer contract:
 * HEAD -> MODL -> nested CNTB/PRIM/CNTE -> END:, with ATT NEW/END names
 * matching CNTB review names in export-tree pre-order.
 */
export function assertRvmChunkHierarchy(rvmBuffer, attText, exportModel = {}) {
  ...
  return {
    ...
    chunkOrder: 'HEAD, MODL, nested CNTB/PRIM/CNTE, END:'
  };
}
