...
const ACTIVE_CACHE_KEY = 'support-ringless-input-panel-revamp-20260624';
...
for (const [name, source] of [['build-pages.mjs', buildScript], ['app-loader.js', appLoader]]) {
  assert.ok(source.includes(ACTIVE_CACHE_KEY), name + ' must use the active ringless input panel revamp cache key');
...
for (const key of [SOURCE_INDEX_CACHE_KEY, 'support-axis-transform-generalized-20260624', ACTIVE_CACHE_KEY]) assert.ok(buildScript.includes(key), 'Pages build must include cache key ' + key);
