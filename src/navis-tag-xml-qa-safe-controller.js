...
if (document.readyState === 'loading') {
  window.addEventListener('DOMContentLoaded', initSafeNavisXmlQa, { once: true });
} else {
  initSafeNavisXmlQa();
}
...
function refreshQaPanel() {
  ...
  const counts = qa.querySelector('.navis-xml-qa-counts');
  const status = qa.querySelector('.navis-xml-qa-status');
  const warnings = qa.querySelector('.navis-xml-qa-warnings');
  const preview = qa.querySelector('.navis-xml-preview');

  if (counts) counts.innerHTML = countPills(report);
  if (status) {
  ...
  if (warnings) {
    warnings.innerHTML = report.warnings.length
  ...
  if (preview) {
    ...
    const area = preview.querySelector('textarea');
    if (state.previewOpen && area) area.value = buildPreviewXml(tags);
  }

  bindQaButtons(qa, report);
...
function bindQaButtons(qa, report) {
  const previewButton = qa.querySelector('[data-qa-action="preview"]');
  if (previewButton) {
  ...
  const copyButton = qa.querySelector('[data-qa-action="copy"]');
  if (copyButton) {
    copyButton.onclick = async () => {
      ...
      try {
        ...
        const preview = qa.querySelector('.navis-xml-preview');
        const area = qa.querySelector('.navis-xml-preview textarea');
        if (preview) preview.hidden = false;
        if (area) area.value = xml;
  ...
  const reportButton = qa.querySelector('[data-qa-action="report"]');
  if (reportButton) {
...
function injectStyles() {
  ...
  style.textContent = `
    ...
    .navis-xml-qa-status { color: #a9bed3; font-size: 11px; }
    .navis-xml-qa-status.warning { color: #ffcf7a; }
    .navis-xml-qa-counts {
      display: flex;
      flex-wrap: wrap;
      gap: 6px;
    ...
    .navis-xml-qa-counts span {
      ...
      align-items: center;
      padding: 4px 7px;
      border: 1px solid rgba(145, 205, 255, .2);
      border-radius: 999px;
      background: rgba(22, 43, 65, .72);
      color: #dcecff;
      font-size: 11px;
    }
    .navis-xml-qa-counts b { color: #87d5ff; }
