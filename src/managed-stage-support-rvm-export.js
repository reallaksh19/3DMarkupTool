...
export function buildManagedStageSupportRvmExportNodes(profile, options = {}) {
  ...
  return {
    schema: 'ManagedStageSupportRvmExport.v1',
    ...
    nodes: supportNodes,
    policy: 'managed-stage ATTA/support records are emitted to RVM as Review-safe code-8 cylinder support overlay symbols; source POS/SUPPORTCOORD remains the anchor coordinate'
  };
}
...
function supportElementNode(record, adapted, visual, index, options = {}) {
  ...
  const rodRadius = clamp(odMm * 0.055, 2.5, 12);
  const supportName = record.attributes?.NAME || record.name || `SUPPORT_${index + 1}`;
  const primitives = [];

  if (distance(sourceCenter, visualCenter) > EPS_MM) {
    primitives.push(supportCylinder({
      name: `${supportName}_CLUSTER_OFFSET_CONNECTOR`,
      localName: 'support-cluster-offset-connector',
      startMm: sourceCenter,
      endMm: visualCenter,
      radiusMm: Math.max(odMm * 0.035, 2),
      record,
      visual,
  ...
  if (visual.family === 'SPRING_CAN') {
    primitives.push(...springCanRods(supportName, visualCenter, odMm, record, visual));
  } else if (visual.fallbackCrossRods || visual.popupRequired) {
    const fallback = fallbackCrossRods(supportName, visualCenter, Math.max(odMm * 1.5, genericLength), rodRadius, record, visual, visual.popupRequired);
    ...
    for (const side of visual.coneSides || []) {
      ...
      const outward = side.pointsTowardCenter === false ? scale(sideVec, -1) : sideVec;
      const outer = add(tip, scale(outward, length));
      primitives.push(supportCylinder({
        name: `${supportName}_${safeName(side.role || 'SUPPORT')}_${axisToken(side.axis)}`,
        localName: side.role || 'support-direction-rod',
        startMm: outer,
        endMm: tip,
        radiusMm: rodRadius,
        record,
        visual,
        role: side.role || 'support-direction-rod',
        extra: {
...
function supportCylinder({ name, localName, startMm, endMm, radiusMm, record, visual, role, extra = {} }) {
  ...
  return {
    ...
    orientationAssumption: 'Support/ATTA record exported as Review-safe code-8 cylinder overlay symbol using staged support mapping rules'
  };
}

function springCanRods(name, center, odMm, record, visual) {
...
function fallbackCrossRods(name, center, length, radius, record, visual, warning = false) {
  ...
  return pairs.map(([a, b], index) => supportCylinder({
    name: `${name}_${warning ? 'WARNING' : 'FALLBACK'}_X_ROD_${index + 1}`,
    localName: warning ? `support-warning-x-rod-${index + 1}` : `support-fallback-x-rod-${index + 1}`,
    startMm: add(center, a),
    endMm: add(center, b),
    radiusMm: radius,
    record,
    visual,
    role: warning ? 'support-warning-x-rod' : 'support-fallback-x-rod',
