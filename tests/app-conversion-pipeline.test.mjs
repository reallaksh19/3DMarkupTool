...
assert.match(source, /defaultConvertInputXmlToGlb/);
assert.match(source, /defaultConvertInputXmlToRvmAtt/);
assert.match(source, /defaultCreateRvmPreviewScene/);
assert.match(source, /dependencyPolicy:\s*'static-defaults-with-override'/);
assert.doesNotMatch(source, /import\('\.\/rvm-converter\.js\?v=professional-viewer-3'\)/);
