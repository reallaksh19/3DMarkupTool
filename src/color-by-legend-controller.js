...
function installStyles() {
  if (document.getElementById('colorByLegendStyles')) return;
  const style = document.createElement('style');
  style.id = 'colorByLegendStyles';
  style.textContent = `
    .color-legend-panel {
      position: absolute;
      left: 18px;
      ...
      gap: 10px;
      padding: 12px;
      border: 1px solid rgba(114, 178, 245, 0.32);
      border-radius: 16px;
      background: linear-gradient(180deg, rgba(10, 22, 34, 0.94), rgba(7, 14, 23, 0.91));
    ...
    .color-legend-head {
      ...
      gap: 10px;
      border-bottom: 1px solid rgba(148, 185, 230, 0.16);
      padding-bottom: 8px;
    }
    ...
    .color-legend-close {
      appearance: none;
      border: 1px solid rgba(150, 190, 232, 0.24);
    ...
    .color-legend-foot {
      ...
      color: #7f93a8;
      font-size: 10px;
      border-top: 1px solid rgba(148, 185, 230, 0.14);
      padding-top: 8px;
    }

    body.props-open .color-legend-panel {
      right: 372px;
      left: auto;
    }
...
function ensurePanel() {
  ...
  const viewer = el('viewer') || document.body;
...
function shouldSkipObject(object) {
  ...
  if (!object.material || object.material.map) return true;
...
function legendEntryForData(data, mode) {
  const type = data.TYPE || data.type || 'Object';

  if (mode === 'lineNo') {
    const value = data.lineNo || data.LINE_NO || data.node || data.NODE || 'Unavailable';
    const label = value === 'Unavailable' ? 'Unavailable / No Line' : `Line ${value}`;
    return { key: `line:${value}`, label, color: colorFromString(value) };
  }
  ...
  if (mode === 'componentType') {
    ...
    if (componentType.includes('VALVE')) return { key: 'component:valve', label: 'Valve', color: MATERIAL_COLORS.valve };
    if (componentType.includes('BEND')) return { key: 'component:bend', label: 'Bend', color: MATERIAL_COLORS.bend };
    if (componentType.includes('RIGID') || componentType.includes('FLANGE')) return { key: 'component:rigid', label: 'Rigid / Flange', color: MATERIAL_COLORS.rigid };
    if (type === 'SUPPORT_RESTRAINT') return { key: 'component:support', label: 'Support / Restraint', color: MATERIAL_COLORS.supportExpected };
    return { key: 'component:pipe', label: 'Pipe / Component', color: MATERIAL_COLORS.pipe };
  }

  if (mode === 'supportSource') {
    const sourceClass = String(data.sourceClass || data.SOURCE_CLASS || data.source || data.SOURCE || '').toUpperCase();
    if (sourceClass.includes('ACTUAL') || sourceClass.includes('INPUTXML')) {
      return { key: 'support:actual', label: 'Actual / InputXML', color: MATERIAL_COLORS.supportActual };
    }
    if (sourceClass.includes('EXPECTED') || sourceClass.includes('ISONOTE')) {
      return { key: 'support:expected', label: 'Expected / ISONOTE', color: MATERIAL_COLORS.supportExpected };
    }
    if (type === 'COMPONENT') return { key: 'support:component', label: 'Component / Pipe', color: MATERIAL_COLORS.pipe };
    return { key: 'support:unavailable', label: 'Unavailable Source', color: MATERIAL_COLORS.unavailable };
  }

  if (mode === 'sourceMode') {
    const value = data.sourceMode || data.SOURCE_MODE || data.source || data.SOURCE || type || 'source';
    return { key: `source:${value}`, label: String(value), color: colorFromString(value) };
  }
...
function hexToCss(hex) {
  return `#${Number(hex || MATERIAL_COLORS.unavailable).toString(16).padStart(6, '0')}`;
}

function collectLegendRows(mode) {
  ...
  scene.traverse((object) => {
    ...
    const entry = legendEntryForData(data, mode);
    if (!entry) return;
    const existing = groups.get(entry.key) || { ...entry, count: 0 };
    existing.count += 1;
    groups.set(entry.key, existing);
  });
...
function renderLegend() {
  ...
  const rows = collectLegendRows(mode);
  ...
  modeLabel.textContent = COLOR_MODE_LABELS[mode] || mode;
  summary.textContent = `${rows.length} categor${rows.length === 1 ? 'y' : 'ies'} / ${rows.reduce((sum, row) => sum + row.count, 0)} meshes`;

  if (!rows.length) {
    list.innerHTML = '<div class="color-legend-empty">No colored model objects found yet. Load/convert a model, then change Color By again.</div>';
  } else {
    ...
    `).join('') + (hiddenCount ? `
      <div class="color-legend-empty">+ ${hiddenCount} more categor${hiddenCount === 1 ? 'y' : 'ies'} not shown</div>
    ` : '');
  }
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', installColorByLegend, { once: true });
} else {
  installColorByLegend();
}
