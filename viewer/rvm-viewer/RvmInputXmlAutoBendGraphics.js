...
function isSourceBend(seg) {
  return Boolean(seg?.sourceBend) || ['BEND', 'ELBO', 'ELBOW'].includes(up(seg?.sourceType));
}
...
function lineCorner(pA, dA, pB, dB, fallbackGap) {
  const a = dA.dot(dA);
  const b = dA.dot(dB);
  const c = dB.dot(dB);
  const w0 = pA.clone().sub(pB);
  const d = dA.dot(w0);
  const e = dB.dot(w0);
  const denom = a * c - b * b;
  if (Math.abs(denom) < 1e-8) return pA.clone().add(pB).multiplyScalar(0.5);
  const sc = (b * e - c * d) / denom;
  const tc = (a * e - b * d) / denom;
  const ca = pA.clone().add(dA.clone().multiplyScalar(sc));
  const cb = pB.clone().add(dB.clone().multiplyScalar(tc));
  if (ca.distanceTo(cb) > fallbackGap) return pA.clone().add(pB).multiplyScalar(0.5);
  return ca.add(cb).multiplyScalar(0.5);
}
function setTrimForItem(mapStart, mapEnd, item, point) {
  if (item.endName === 'p0') mapStart.set(item.seg, point);
  else mapEnd.set(item.seg, point);
}
...
export function buildInputXmlNodeGraph() {
  ...
  components.forEach((node, index) => {
    ...
    edges.push({
      ...
      sourceType,
      sourceBend: ['BEND', 'ELBO', 'ELBOW'].includes(sourceType),
    });
  });
  ...
  const graph = {
    ...
    stats: {
      ...
      sourceBendEdges: edges.filter((seg) => seg.sourceBend).length,
      skippedNoNodeEdge,
      skippedNoGeometry,
    },
  };
...
function shouldGenerateBendAtNode(items, blockedNodes, nodeId) {
  if (blockedNodes.has(nodeId)) return false;
  if (items.length !== 2) return false;
  const [a, b] = items;
  if (a.seg === b.seg) return false;
  if (isSourceBend(a.seg) || isSourceBend(b.seg)) return false;
  return true;
}

function collapseSourceBends(graph, root, trimStart, trimEnd) {
  const blockedNodes = new Set();
  const skipRender = new Set();
  let collapsedSourceBends = 0;
  let sourceBendFallbackPipes = 0;
  let skippedSourceBendCollapse = 0;

  for (const seg of graph.edges.filter(isSourceBend)) {
    blockedNodes.add(seg.fromNode);
    blockedNodes.add(seg.toNode);
    const startItems = graph.byNode.get(seg.fromNode) || [];
    const endItems = graph.byNode.get(seg.toNode) || [];
    const startAdj = startItems.filter((item) => item.seg !== seg && !isSourceBend(item.seg));
    const endAdj = endItems.filter((item) => item.seg !== seg && !isSourceBend(item.seg));
    if (startAdj.length !== 1 || endAdj.length !== 1 || startAdj[0].seg === endAdj[0].seg) {
      sourceBendFallbackPipes += 1;
      continue;
    }

    const a = startAdj[0];
    const b = endAdj[0];
    const pA = graph.nodePoints.get(seg.fromNode) || seg.p0;
    const pB = graph.nodePoints.get(seg.toNode) || seg.p1;
    ...
    const dia = Math.min(a.seg.diameter, b.seg.diameter, seg.diameter);
    const pipeR = Math.max(dia * 0.5, 1);
    const corner = lineCorner(pA, dA, pB, dB, Math.max(dia * 6, pA.distanceTo(pB) * 1.5));
    ...
    const bend = makeArc(dA, dB, corner, bendRadius, pipeR, BEND_MAT, `INPUTXML_AUTO_BEND_${seg.fromNode}_${seg.toNode}`, maxTrimA, maxTrimB);
    if (!bend) {
      skippedSourceBendCollapse += 1;
      sourceBendFallbackPipes += 1;
      continue;
    }

    root.add(bend.mesh);
    setTrimForItem(trimStart, trimEnd, a, bend.p1);
    setTrimForItem(trimStart, trimEnd, b, bend.p2);
    skipRender.add(seg);
    collapsedSourceBends += 1;
  }

  return { blockedNodes, skipRender, collapsedSourceBends, sourceBendFallbackPipes, skippedSourceBendCollapse };
}
...
export function applyInputXmlAutoBendGraphics(viewer, options = {}) {
  ...
  let skippedSourceBendAdjacency = 0;
  let nodeMarkers = 0;

  const sourceBendResult = collapseSourceBends(graph, root, trimStart, trimEnd);
  bends += sourceBendResult.collapsedSourceBends;
  ...
  graph.byNode.forEach((items, id) => {
    ...
    if (!shouldGenerateBendAtNode(items, sourceBendResult.blockedNodes, id)) {
      if (items.some((item) => isSourceBend(item.seg))) skippedSourceBendAdjacency += 1;
      return;
    }
    const [a, b] = items;
    if (!corner) {
  ...
  for (const seg of segs) {
    if (sourceBendResult.skipRender.has(seg)) continue;
  ...
  viewer.inputXmlAutoBendDiagnostics = {
    ...
    collapsedSourceBends: sourceBendResult.collapsedSourceBends,
    sourceBendFallbackPipes: sourceBendResult.sourceBendFallbackPipes,
    skippedSourceBendCollapse: sourceBendResult.skippedSourceBendCollapse,
