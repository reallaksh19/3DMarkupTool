...
const ACTIVE_CACHE_KEY = 'staged-json-review-ui-rvm-fix-20260625';
const SOURCE_INDEX_CACHE_KEY = 'staged-json-review-ui-rvm-fix-20260625';
...
assert.deepEqual([...new Set(htmlKeys)], [SOURCE_INDEX_CACHE_KEY], 'source index keys must use the active stagedJson review fix cache key');
...
for (const [name, source] of [['build-pages.mjs', buildScript], ['app-loader.js', appLoader]]) {
  assert.ok(source.includes(ACTIVE_CACHE_KEY), name + ' must use the active stagedJson review UI/RVM fix cache key');
...
for (const [name, source] of [['safe-ui-bootstrap.js', safeBootstrap], ['static-browser-diagnostics-controller.js', diagnostics]]) assert.ok(source.includes(LEGACY_FIRST_PAINT_CACHE_KEY), name + ' must keep the stable first-paint shell cache key');
assert.ok(buildScript.includes('LEGACY_CACHE_KEYS = Object.freeze(['));
for (const key of ['app-boot-dialog-conversion-hotfix-20260623', 'support-axis-transform-generalized-20260624', 'support-ringless-input-panel-revamp-20260624', ACTIVE_CACHE_KEY]) assert.ok(buildScript.includes(key), 'Pages build must include cache key ' + key);
