...
for (const required of [
  ...
  'hasAttributeBackedCompactNativeRvmSupportOverlay()',
  'hasCompactNativeRvmSupportOverlay(viewer)',
...
assert.ok(
  bridge.includes('hasCode8 && hasNativeCylinder && hasBrowserFallback'),
  'standalone fallback suppression must require code-8 native cylinder fallback evidence, not support-name alone',
);
assert.ok(
  bridge.includes('export {') && bridge.includes('isStandaloneFallbackNativeSupportCylinder'),
  'bridge must export standalone detector helpers for regression visibility',
);
assert.ok(
  viewerEntry.includes('RvmInputXmlSupportGraphicsSetModelBridge.js?v=20260625-rvm-standalone-support-overlay-clean-1'),
  'viewer startup must load the standalone RVM support overlay bridge with a fresh cache key',
);
assert.ok(
  viewerIndex.includes('main-rhbg-fitguard.js?v=20260625-rvm-standalone-support-overlay-clean-1'),
