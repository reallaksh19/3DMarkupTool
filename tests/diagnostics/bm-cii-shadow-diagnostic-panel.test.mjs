...
import { buildDiagnosticPanelViewModel, collectPanelForbiddenFieldHits, validateDiagnosticPanelViewModel } from '../../src/diagnostics/diagnostic-panel-view-model.js';
import { renderShadowDiagnosticPanelHtml } from '../../src/ui/diagnostic-panel/shadow-diagnostic-panel.js';
...
const graph = convertManagedStageJsonToPlantGraph(sourceText, { sourceName: 'bm-cii-managed-stage-full-topology.generated.json', phase: 'Phase 9A shadow diagnostic panel benchmark' });
...
assert.equal(assertTestArtifactAdapterPlanContract(testArtifactPlan).ok, true, 'artifact plan ok');
...
assert.equal(assertDiagnosticCanvasPreviewModelContract(previewModel).ok, true, 'preview model ok');
...
assert.deepEqual(collectPanelForbiddenFieldHits(viewModel), [], 'panel model exposes no geometry/mesh/binary/text payloads');
const html = renderShadowDiagnosticPanelHtml(viewModel);
assert.ok(html.includes('RVM straight-pipe subset byte proof: READY'));
assert.ok(html.includes('RVM full model: NOT READY'));
assert.ok(html.includes('ATT: BLOCKED'));
assert.ok(html.includes('GLB: BLOCKED'));
...
console.log('BM CII shadow diagnostic panel tests passed');
