export const MANAGED_STAGE_SUPPORT_SETTINGS_POPUP_SCHEMA = 'ManagedStageSupportSettingsPopup.v5';
export const MANAGED_STAGE_SUPPORT_SETTINGS_POPUP_CACHE_KEY = '20260625-support-settings-workflow-card-structured';

export function buildManagedStageSupportSettingsPopupModel({ supportUi = {}, isonoteWorkflow = {}, supportDebug = {} } = {}) {
  ...
  const issueCount = Number(isonoteWorkflow.issueCount || supportDebug?.diagnostics?.mapperPreflightIssueCount || 0);
  ...
  return {
    ...
    northSourceAxis,
    northCanvasAxis,
    detectedSupportCount,
    disabled,
    modal: true,
    nativeDialog: true,
    mainPanelMode: 'workflow-card-summary-only',
    summaryText,
    isonoteSummaryText,
...
export function installManagedStageSupportSettingsPopupUi({ doc = globalThis.document, win = globalThis.window } = {}) {
  ...
  const refresh = () => {
    const model = buildManagedStageSupportSettingsPopupModel({
      ...
      supportDebug: win?.__3D_MARKUP_SUPPORT_DEBUG_LAST_REPORT__ || {}
    });
    renderSupportSettingsShell(shell, dialog, model, win);
    hydrateIcons(doc, win);
  ...
  if (shell.dataset.supportSettingsPopupInstalled !== 'true') {
    shell.addEventListener?.('click', (event) => {
      ...
      if (action === 'open') setPopupOpen(dialog, true, doc);
      if (action === 'debug') runSupportDebugDump(win, doc, shell);
    });
  ...
  if (dialog.dataset.supportSettingsPopupInstalled !== 'true') {
    dialog.addEventListener?.('click', (event) => {
      const actionNode = event?.target?.closest?.('[data-support-settings-action]');
      const action = actionNode?.getAttribute?.('data-support-settings-action');
      if (action === 'apply-close' || action === 'close' || event.target === dialog) {
        event.preventDefault?.();
        setPopupOpen(dialog, false, doc);
      }
      if (action === 'debug') {
        event.preventDefault?.();
        runSupportDebugDump(win, doc, shell);
      }
    });
    ...
    doc?.addEventListener?.('managed-stage:support-source-ui-ready', refresh);
    win?.addEventListener?.('managed-stage:isonote-workflow-apply', refresh);
    win?.addEventListener?.('managed-stage:support-debug-button-ready', refresh);
...
function ensureSupportSettingsLauncherShell(doc, parent) {
  ...
  shell.innerHTML = [
    ...
    '<button type="button" class="support-mapping-settings-open" data-support-settings-action="open" aria-haspopup="dialog" aria-controls="supportMappingSettingsDialog"><i data-lucide="settings-2"></i><span>Open Support Mapping / ISONOTE</span></button>',
    '<button type="button" class="support-mapping-settings-debug" data-support-settings-action="debug" data-support-debug-log-action="dump"><i data-lucide="bug"></i><span>Dump Support Debug</span></button>',
    '</div>',
...
function ensureSupportSettingsDialog(doc) {
  ...
  if (!dialog) {
    ...
    dialog.innerHTML = [
      ...
      '<section data-support-settings-mapper-host><h4>AXIS / MAPPER</h4></section>',
      '<section data-support-settings-isonote-host><h4>ISONOTE / DIAGNOSTICS</h4><div data-support-settings-diagnostics class="support-settings-diagnostics"></div></section>',
      '</div>',
      '<div class="support-mapping-settings-popup-footer">',
      '<button type="button" class="primary" data-support-settings-action="apply-close">Apply & Close</button>',
      '<button type="button" data-support-settings-action="close">Close</button>',
      '<button type="button" data-support-settings-action="debug" data-support-debug-log-action="dump">Dump Support Debug</button>',
      '</div>'
    ].join('');
...
function renderSupportSettingsShell(shell, dialog, model, win) {
  ...
  const mode = shell.querySelector?.('[data-support-settings-mode]');
  const preset = shell.querySelector?.('[data-support-settings-preset]');
  const north = shell.querySelector?.('[data-support-settings-north]');
  const count = shell.querySelector?.('[data-support-settings-count]');
  ...
  if (diagnostics) diagnostics.textContent = diagnosticsText(model, win?.__3D_MARKUP_SUPPORT_DEBUG_LAST_REPORT__ || {});
}
...
function adoptSupportSettingControls(doc, dialog) {
  ...
  adoptLabeledControl(doc, controlsHost, 'supportMode', 'Mode');
  adoptLabeledControl(doc, controlsHost, 'singleAxisDecision', 'Single-axis unresolved decision');
  adoptLabeledControl(doc, mapperHost, 'supportMapperPreset', 'Preset');
  adoptLabeledControl(doc, mapperHost, 'supportNorthAxis', 'North axis selector');
  ...
  adoptTextarea(doc, isonoteHost, 'isonoteText', 'ISONOTE text');
...
function adoptLabeledControl(doc, host, id, labelText) {
  ...
  label.classList?.remove('conversion-collapsible-content');
  label.hidden = false;
  label.style.display = '';
...
function adoptTextarea(doc, host, id, labelText) {
  ...
  label.classList?.remove('conversion-collapsible-content', 'sideload-collapsible-content');
  label.hidden = false;
  label.style.display = '';
  label.removeAttribute?.('aria-hidden');
  host.appendChild(label);
...
function adoptNode(host, node) {
  ...
  node.classList?.remove?.('conversion-collapsible-content');
  node.hidden = false;
  node.style.display = '';
...
function runSupportDebugDump(win, doc, shell) {
  const api = win?.__3D_MARKUP_SUPPORT_DEBUG_LOG__;
  const summary = shell?.querySelector?.('[data-support-settings-launcher-summary]');
  if (typeof api?.dump === 'function') {
    api.dump({ reason: 'support-settings-button', automatic: false });
    if (summary) summary.textContent = 'Support debug dumped to Log.';
    return true;
  }
  const text = 'Support debug logger is not ready yet.';
  if (summary) summary.textContent = text;
  const log = doc?.getElementById?.('log');
  if (log) log.textContent += `[${new Date().toLocaleTimeString()}] ${text}\n`;
  return false;
}

function diagnosticsText(model, report = {}) {
...
function installSupportSettingsModalStyles(doc) {
  ...
  style.textContent = `
    ...
    .support-mapping-settings-launcher { display: grid; gap: 7px; }
    .support-mapping-summary-grid { display: grid; grid-template-columns: auto minmax(0,1fr); gap: 4px 8px; padding: 8px; border: 1px solid rgba(148,163,184,.22); border-radius: 10px; background: rgba(2,6,23,.38); font-size: 11px; }
    .support-mapping-summary-grid span { color: #94a3b8; }
    .support-mapping-summary-grid strong { min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; color: #e0f2fe; }
    .support-mapping-settings-actions { display: grid; grid-template-columns: minmax(0,1fr) minmax(0,.82fr); gap: 7px; }
    .support-mapping-settings-open, .support-mapping-settings-debug { width: 100%; min-height: 34px; border-radius: 8px; font-weight: 800; cursor: pointer; display: inline-flex; align-items: center; justify-content: center; gap: 7px; }
    .support-mapping-settings-open { border: 1px solid rgba(96,165,250,.55); background: rgba(30,64,175,.45); color: #dbeafe; }
    .support-mapping-settings-debug { border: 1px solid rgba(251,191,36,.55); background: rgba(120,53,15,.45); color: #fde68a; }
    ...
    .support-mapping-settings-popup { position: fixed; z-index: 9999; left: 50%; top: 50%; transform: translate(-50%, -50%); width: min(1180px, calc(100vw - 36px)); max-width: none; max-height: min(86vh, 880px); overflow: auto; border: 1px solid rgba(148,163,184,.36); border-radius: 16px; background: #0f172a; color: #e5e7eb; box-shadow: 0 24px 80px rgba(0,0,0,.55); padding: 14px; margin: 0; }
    ...
    .support-mapping-settings-popup-header h3 { margin: 0 0 3px; font-size: 16px; text-transform: uppercase; letter-spacing: .035em; }
    ...
    .support-mapping-settings-popup-grid { display: grid; grid-template-columns: minmax(220px, .72fr) minmax(320px, 1fr) minmax(360px, 1.25fr); gap: 12px; align-items: start; }
    ...
    .support-settings-diagnostics { white-space: pre-line; margin-bottom: 10px; padding: 8px; border: 1px solid rgba(148,163,184,.22); border-radius: 8px; background: rgba(2,6,23,.38); color: #bfdbfe; font-size: 11px; line-height: 1.45; }
    .support-mapping-settings-popup-footer { display: flex; align-items: center; justify-content: flex-end; gap: 8px; margin-top: 12px; padding-top: 12px; border-top: 1px solid rgba(148,163,184,.22); }
    body.support-settings-modal-open { overflow: hidden; }
    @media (max-width: 900px) { .support-mapping-settings-popup { width: calc(100vw - 24px); max-height: calc(100vh - 24px); } .support-mapping-settings-popup-grid, .support-mapping-settings-actions { grid-template-columns: 1fr; } }
  `;
  doc.head?.appendChild(style);
}

function hydrateIcons(doc, win) {
  try {
    const lucide = win?.lucide || globalThis.lucide;
    if (lucide?.createIcons) lucide.createIcons();
  } catch (_) {
    /* lucide is optional in tests and fallback shells */
  }
}
