...
assert.equal(assertWriterAdapterAudit(state.writerAdapterAudit, { ok: true, rvmWriterReady: true, rvmPipeBendSubsetTestByteReady: true, rvmPlannedCylinderCount: 19, rvmPlannedTorusCount: 0, testByteEligibleTorusCount: 7, testByteEligibleBendTorusCount: 7, productionReadyTorusCount: 0, deferredFlangeWriterCount: 8, flangeWriterReadyCount: 0, flangeTestByteEligibleCount: 0, blockedFlangeWriterCount: 0, blockedValveWriterCount: 6, blockedBendWriterCount: 0, deferredSupportWriterCount: 12, deferredBendTorusWriterCount: 0, writerCallCount: 0, binaryPayloadCount: 0, textPayloadCount: 0, glbPayloadCount: 0 }).ok, true);
assert.equal(state.writerAdapterPlan.rvmAdapter.testByteEligibleItems.length, 7);
assert.equal(state.writerAdapterPlan.rvmAdapter.deferredItems.filter((entry) => entry.family === 'flange').length, 8);
