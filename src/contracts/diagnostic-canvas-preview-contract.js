...
export function validateDiagnosticCanvasPreviewModelContract(model) {
  ...
  if (model?.schema !== DIAGNOSTIC_CANVAS_PREVIEW_MODEL_SCHEMA) errors.push(`schema must be ${DIAGNOSTIC_CANVAS_PREVIEW_MODEL_SCHEMA}`);
  if (!model?.graphId) errors.push('graphId is required');
  if (!model?.units) errors.push('units is required');
  if (model?.mode !== MODE) errors.push(`mode must be ${MODE}`);
  if (!Array.isArray(model?.sourceSchemas)) errors.push('sourceSchemas array is required');
  if (!model?.artifactStatusBanner || typeof model.artifactStatusBanner !== 'object') errors.push('artifactStatusBanner object is required');
  if (!Array.isArray(model?.summaryCards)) errors.push('summaryCards array is required');
  if (!Array.isArray(model?.previewItems)) errors.push('previewItems array is required');
  if (!Array.isArray(model?.blockedBadges)) errors.push('blockedBadges array is required');
