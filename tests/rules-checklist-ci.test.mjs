...
function assertBefore(source, firstText, secondText, message) { const before = source.indexOf(firstText); const after = source.indexOf(secondText); assert.notEqual(before, -1, message + ': missing first marker ' + firstText); assert.notEqual(after, -1, message + ': missing second marker ' + secondText); assert.ok(before < after, message); }
function extractActiveQueryKeys(html) { const keys = []; for (const match of html.matchAll(/v=([^"'&]+)/g)) keys.push(match[1]); return keys.filter((key) => key.includes('202606')); }
...
for (const [name, source] of [['safe-ui-bootstrap.js', safeBootstrap], ['static-browser-diagnostics-controller.js', diagnostics]]) assert.ok(source.includes(LEGACY_FIRST_PAINT_CACHE_KEY));
assert.ok(buildScript.includes('LEGACY_CACHE_KEYS = Object.freeze(['));
for (const key of ['app-boot-dialog-conversion-hotfix-20260623', 'support-axis-transform-generalized-20260624', 'support-ringless-input-panel-revamp-20260624', 'staged-json-review-ui-rvm-fix-20260625', 'input-load-controls-restored-20260626', 'input-postbootstrap-reassert-20260626', 'input-root-owner-20260626', ACTIVE_CACHE_KEY]) assert.ok(buildScript.includes(key), 'Pages build must include cache key ' + key);
...
assert.ok(shellCss.includes('INPUT is not a collapsible workflow-card'));
...
assert.ok(index.includes('data-section="support-mapping"'));
assert.ok(index.includes('conversionOptionsCompatRoot'));
assert.ok(index.includes('id="loadUnifiedModelFileBtn"'));
assert.ok(index.includes('data-input-root="persistent"'));
assert.ok(!/class="panel-section workflow-card[^"]*"[^>]*data-section="input"/.test(index));
...
assert.ok(appLoader.includes('loadManagedStageDecorators'));
