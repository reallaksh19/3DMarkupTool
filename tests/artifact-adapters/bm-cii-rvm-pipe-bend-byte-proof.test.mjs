...
for (const runtimePath of ['src/app.js', 'src/app-loader.js', 'src/safe-ui-loader.js', 'src/managed-stage-json-ui-controller.js', 'src/managed-stage-rvm-converter.js']) {
  ...
  assert.equal(source.includes('rvm-flange-cylinder-test-byte-writer'), false, `${runtimePath} must not import test-only flange writer`);
...
assert.equal(writerSource.includes('Phase 11C-B'), false, 'production rvm-writer is not modified for Phase 11C-B');
