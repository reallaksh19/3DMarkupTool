...
assert.match(staticShellCss, /panel-section\[data-section="input"\]\s*>\s*\.file-drop[\s\S]*display:\s*grid\s*!important/, 'Choose stagedJson must be compact and statically visible');
assert.match(staticShellCss, /panel-section\[data-section="input"\]\s*>\s*\.file-drop[\s\S]*min-height:\s*32px/, 'Choose stagedJson must not regress to a tall drop zone');
