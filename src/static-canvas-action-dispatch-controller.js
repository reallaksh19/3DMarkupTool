...
// Repairs real canvas click/drag ownership for Area Select, Section Box,
// Isolate, and Hide after the viewer canvas is created. This is UI-only:
// it does not change parser/export/model data and inspects the scene only
// after the user invokes a canvas action.
...
const RUNTIME_EVENTS = [
  ...
  'viewer:area-select-ready',
  'viewer:section-box-ready',
  'viewer:viewpad-tools-ready'
];
...
function patchAreaSelectApi() {
  ...
  const originals = {
    activate: typeof api.activate === 'function' ? api.activate.bind(api) : null,
    deactivate: typeof api.deactivate === 'function' ? api.deactivate.bind(api) : null
  };
  api.activate = (options = {}) => {
    originals.activate?.(options);
    return activateAreaSelect(options);
  };
  api.deactivate = (message = '') => {
    originals.deactivate?.(message);
    return deactivateAreaSelect(message);
  };
...
function actionTargets() {
  const area = sanitizeTargets(state.areaSelected);
...
function projectedObjectCenter(object, camera, viewport) {
  ...
  return {
    x: viewport.left + ((center.x + 1) / 2) * viewport.width,
    y: viewport.top + ((1 - center.y) / 2) * viewport.height
  };
}
