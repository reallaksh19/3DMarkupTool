...
function evaluateRenderedSpanContinuity(spanRoles) {
  ...
  return { ok: gaps.length === 0, spanCount: spanRoles.length, coverageStart: round(start), coverageEnd: round(cursor), coverageLength: round(cursor - start), tolerance: round(tolerance), gaps, overlaps };
}
...
function buildAudit(conversion, groups) {
  ...
  return {
    ...
    policies: { screenshotStyleVisualArtifact: true, actualThreeSceneDerived: true, deterministicSvgSnapshots: true, proportionalFallback: true, rvmWriterUnaffected: true, asmeDimensionalDatabaseBacked: false },
    issues,
    catalogueGroups: groups.map((entry) => ({ componentId: entry.componentId, groupName: entry.groupName, componentClass: entry.componentClass, componentType: entry.componentType, visualKey: entry.visualKey, visualRecipeId: entry.visualRecipeId, visualCatalogSchema: entry.visualCatalogSchema, meshCount: entry.meshCount, roleObjectCount: entry.roleObjectCount, spanContinuity: entry.spanContinuity, visualProfile: entry.visualProfile, issues: entry.issues }))
  };
}
