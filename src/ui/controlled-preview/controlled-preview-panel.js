...
export function renderControlledPreviewHtml(controlledPreviewModel, options = {}) {
  ...
  if (!controlledPreviewModel || controlledPreviewModel.schema !== 'ControlledPreviewModel.v1') {
    return wrapPanel('<div class="controlled-preview-panel__empty">Controlled preview unavailable: diagnostic/artifact state not available.</div>');
  }
  const readiness = controlledPreviewModel.artifactReadiness || {};
  const strip = [
    { label: 'pipe segments ready', value: controlledPreviewModel.straightPipeSubsetPreview?.length || 0 },
    { label: 'flanges blocked', value: readiness.blockedFlangeCount || 0 },
    { label: 'valves blocked', value: readiness.blockedValveCount || 0 },
    { label: 'bends blocked', value: readiness.blockedBendCount || 0 },
    { label: 'supports deferred', value: readiness.deferredSupportCount || 0 }
  ].map((item) => `<span class="controlled-preview-panel__strip-item"><strong>${escapeHtml(item.value)}</strong> ${escapeHtml(item.label)}</span>`).join('');
  ...
  const traceFooter = Array.isArray(controlledPreviewModel.sourceTracePreview) && controlledPreviewModel.sourceTracePreview.length > traceLimit
    ? `<p class="controlled-preview-panel__note">Showing ${escapeHtml(traceLimit)} of ${escapeHtml(controlledPreviewModel.sourceTracePreview.length)} source trace rows.</p>`
    : '';
  return wrapPanel(`
    <section class="controlled-preview-panel__content" aria-label="Controlled shadow preview">
      <h3>Controlled shadow preview — diagnostic/artifact state only, not geometry.</h3>
      <div class="controlled-preview-panel__banner">
        <strong>RVM straight-pipe subset: ${escapeHtml(readiness.rvmStraightPipeSubsetReady ? 'READY' : 'NOT READY')}</strong><br>
        <span>RVM full model: ${escapeHtml(readiness.rvmFullModelReady ? 'READY' : 'NOT READY')}</span><br>
        <span>ATT: ${escapeHtml(readiness.attReady ? 'READY' : 'BLOCKED')}</span><br>
        <span>GLB: ${escapeHtml(readiness.glbReady ? 'READY' : 'BLOCKED')}</span>
      </div>
      <div class="controlled-preview-panel__strip">${strip}</div>
      <div class="controlled-preview-panel__schematic" role="img" aria-label="Schematic diagnostic preview, not geometry">${cells}</div>
      <table class="controlled-preview-panel__trace" aria-label="Controlled preview source trace">
        <thead><tr><th>Item</th><th>Family</th><th>Status</th><th>Readiness</th><th>Message</th></tr></thead>
        <tbody>${rows}</tbody>
      </table>
      ${traceFooter}
    </section>`);
...
export function unmountControlledPreview(container) {
