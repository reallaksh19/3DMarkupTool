...
const POPUP_SCHEMA = 'RvmPostAxisTransformPopupController.v1';
...
export function installRvmPostAxisTransformPopupController({ win = globalThis.window, doc = globalThis.document } = {}) {
  ...
  const api = {
    ...
    getConfig: () => getActiveConfig(win),
    open: () => openDialog(win, doc),
    save: (config) => saveConfig(win, config)
  };
...
function renderDialog(win, doc, dialog) {
  ...
  const presetRows = Object.values(RVM_POST_AXIS_TRANSFORM_PRESETS).map((preset) => ({
    ...preset,
    selected: preset.id === config.presetId
  }));
  const selectedPreset = RVM_POST_AXIS_TRANSFORM_PRESETS[config.presetId] || RVM_POST_AXIS_TRANSFORM_PRESETS.navisObservedWnt;
  dialog.innerHTML = `
    <form method="dialog" class="rvm-post-axis-transform-card">
      <div class="rvm-post-axis-transform-head">
        <div>
          <h3>RVM Navis Axis Transform</h3>
          <p>Applied after the complete RVM export model is generated and before writeRvm()/ATT/preview consume it.</p>
        </div>
      ...
      <label class="field"><span>Preset</span><select id="rvmPostAxisTransformPreset">
        ${presetRows.map((preset) => `<option value="${escapeHtml(preset.id)}" ${preset.selected ? 'selected' : ''}>${escapeHtml(preset.label)}</option>`).join('')}
      </select></label>
      <div class="rvm-post-axis-transform-proof">
        <h4>Observed mapping</h4>
        <table><tbody>
          <tr><th>Navis N</th><td>Canvas +Y</td></tr>
          <tr><th>Navis Top</th><td>Canvas +Z</td></tr>
          <tr><th>Navis W</th><td>Canvas -X</td></tr>
        </tbody></table>
        ...
        <p><strong>Description:</strong> ${escapeHtml(selectedPreset.description)}</p>
      </div>
      <div class="rvm-post-axis-transform-actions">
        <button id="rvmPostAxisTransformSaveBtn" type="button" class="primary">Save Transform Config</button>
        <button id="rvmPostAxisTransformOffBtn" type="button" class="ghost">Disable</button>
      </div>
    </form>
  `;
  const presetSelect = dialog.querySelector('#rvmPostAxisTransformPreset');
  const updateSummary = () => {
    const preset = RVM_POST_AXIS_TRANSFORM_PRESETS[presetSelect.value] || RVM_POST_AXIS_TRANSFORM_PRESETS.navisObservedWnt;
  ...
  dialog.querySelector('#rvmPostAxisTransformSaveBtn')?.addEventListener('click', () => {
    ...
    saveConfig(win, {
      ...
      enabled: dialog.querySelector('#rvmPostAxisTransformEnabled')?.checked !== false,
      presetId: preset.id,
  ...
  dialog.querySelector('#rvmPostAxisTransformOffBtn')?.addEventListener('click', () => {
    saveConfig(win, { ...DEFAULT_RVM_POST_AXIS_TRANSFORM_CONFIG, enabled: false, presetId: 'off' });
    dialog.close?.();
  });
}

function getActiveConfig(win) {
  return resolveRvmPostAxisTransformConfig(readStoredConfig(win) || DEFAULT_RVM_POST_AXIS_TRANSFORM_CONFIG, win);
}

function saveConfig(win, config) {
  const resolved = resolveRvmPostAxisTransformConfig(config, win);
  try { win.localStorage?.setItem?.(STORAGE_KEY, JSON.stringify(resolved)); } catch (_) {}
  win.__3D_MARKUP_RVM_POST_AXIS_TRANSFORM_CONFIG__ = resolved;
  win.dispatchEvent?.(new CustomEvent('managed-stage:rvm-post-axis-transform-config-changed', { detail: resolved }));
  return resolved;
}

function readStoredConfig(win) {
  try {
    const text = win.localStorage?.getItem?.(STORAGE_KEY);
