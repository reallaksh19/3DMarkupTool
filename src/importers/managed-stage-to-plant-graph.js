...
export function auditManagedStageToPlantGraph(sourceText, graph, options = {}) {
  ...
  return {
    ...
    bendArcEvidenceCount: componentItems.filter((item) => item.bendEvidence?.arcCenter && item.bendEvidence?.normal).length,
...
function componentItem(record, a, id, routes, routeByRecordId) {
  ...
  const item = hasCatalogueRef ? {
    ...
    kind: 'component',
    tagged: true,
    catalogueRef: compact({ catalogue: CATALOGUE.id, family: explicitFamily, type: explicitType, nps, schedule }),
...
function componentPlacement(record, a, routes, route) {
  ...
  if (from && to) return compact({ fromNode: from, toNode: to, route, position });
  const node = nodeId(value(a, 'NODE', 'AT_NODE', 'FROM_NODE', 'TO_NODE'));
  const out = {};
...
function bendEvidence(a) {
  return compact({
    ...
    arcCenter: point(value(a, 'BEND_ARC_CENTER', 'ARC_CENTER', 'TRUE_BEND_CENTER')),
    normal: unitPoint(value(a, 'BEND_PLANE_NORMAL', 'ARC_NORMAL', 'BEND_NORMAL')),
    startTangent: unitPoint(value(a, 'BEND_START_TANGENT', 'START_TANGENT', 'IN_TANGENT')),
    endTangent: unitPoint(value(a, 'BEND_END_TANGENT', 'END_TANGENT', 'OUT_TANGENT')),
...
function countSourceRecords(records, source) {
  const sourceStats = source?.stats && typeof sourceStats === 'object' ? source.stats : {};
  const counts = { component: 0, pipe: 0, flange: 0, valve: 0, bend: 0, support: 0 };
  for (const { record } of records) {
    if (isSupport(record)) { counts.support += 1; continue; }
    if (!isComponent(record) && !isPipeRecord(record)) continue;
    counts.component += isPipeRecord(record) ? 0 : 1;
    if (isPipeRecord(record)) counts.pipe += 1;
    else if (isFlangeRecord(record)) counts.flange += 1;
    else if (isValveRecord(record)) counts.valve += 1;
    else if (isBendRecord(record)) counts.bend += 1;
  }
  return {
    component: Number(source?.stats?.componentRows ?? source?.stats?.components ?? counts.component),
    pipe: counts.pipe,
    flange: Number(source?.stats?.flanges ?? counts.flange),
    valve: Number(source?.stats?.valves ?? counts.valve),
    bend: Number(source?.stats?.bends ?? counts.bend),
    support: Number(source?.stats?.validRestraints ?? source?.stats?.emittedSupports ?? counts.support)
  };
}

function emptyGraph(options, warnings = []) {
  return { schema: PLANT_MODEL_GRAPH_SCHEMA, id: options.graphId || 'managed-stage-json', project: { name: options.graphId || 'managed-stage-json', units: options.units || 'mm', axisBasis: { authoring: 'canvas-current', rvmExport: 'navis-review' } }, catalogues: [{ ...CATALOGUE }], nodes: [], routes: [], items: [], sourceRefs: [], warnings };
}
