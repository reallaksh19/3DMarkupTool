const APP_LOADER_VERSION = 'input-persistent-root-card-20260629-m';
...
const SUPPORT_AXIS_TRANSFORM_UI_DEBUG_MODULE_URL = `./support-axis-transform-ui-debug-controller.js?v=${APP_LOADER_VERSION}`;
...
async function startViewerApp() {
  ...
  import(APP_MODULE_URL).then(() => { markAppLoaded({ bundled: false }); scheduleIdle(loadCanvasToolModeGuard, 50); scheduleIdle(loadManagedStageJsonUiController, 100); scheduleIdle(loadFreshClipController, POST_APP_IDLE_TIMEOUT_MS); }).catch(handleAppBootError);
}
