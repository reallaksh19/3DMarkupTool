...
assert.match(controller, /stableDrop = modelFileInput\.closest\('\.file-drop'\)/, 'current workflow must use the static .file-drop as the stagedJson load control');
assert.match(controller, /return null;[\s\S]*const button = document\.createElement\('button'\)/, 'runtime duplicate load button must be fallback-only when .file-drop is missing');
assert.doesNotMatch(controller, /host\.insertBefore\(button, sampleButton\)[\s\S]{0,220}return button;\n\}/, 'current workflow must not unconditionally insert a duplicate load button before the sample button');
