...
assert.ok(loader.indexOf('import(MANAGED_STAGE_JSON_UI_MODULE_URL)') < loader.indexOf('loadManagedStageDecorators'), 'managed-stage input shell must load before heavy decorators');

assert.match(controller, /ManagedStageJsonUiController\.v3/);
...
assert.match(controller, /getManagedStageRuntimeModules/);
assert.match(controller, /convertManagedStageJsonToRvmAtt/);
assert.match(controller, /createManagedStagePreviewScene/);
assert.doesNotMatch(controller, /^import \* as THREE/m);
assert.doesNotMatch(controller, /^import \{ convertManagedStageJsonToRvmAtt \}/m);
...
assert.match(controller, /topologyComponentCount:\s*52/);
assert.match(controller, /topologyGeometryComponentCount:\s*40/);
assert.match(controller, /topologySupportCount:\s*12/);
assert.match(controller, /explicitBendRecordCount:\s*7/);
assert.match(controller, /explicitBendDetailCount:\s*7/);
assert.match(controller, /missingExplicitBendDetailCount:\s*0/);
assert.match(controller, /synthetic1p5DTrimBlockedCount:\s*7/);
assert.match(controller, /supportAssociationOnlyCount:\s*12/);
assert.match(controller, /supportTopologyBlockedCount:\s*0/);
assert.match(controller, /supportContinuityEdgeCount:\s*0/);
assert.match(controller, /supportInlineFaceCount:\s*0/);
...
assert.match(controller, /managedStageTopologyDiagnostics/);
assert.match(controller, /ManagedStageVisibleDiagnostics\.v1/);
assert.match(controller, /buildManagedStageVisibleDiagnostics/);
assert.match(controller, /updateManagedStageDiagnosticsPanel/);
assert.match(controller, /Topology proof:/);
assert.match(controller, /Explicit BEND details:/);
assert.match(controller, /Support topology:/);
assert.match(controller, /viewer:managed-stage-json-loaded/);
assert.match(controller, /visibleDiagnostics/);
...
assert.match(sampleController, /ManagedStageBmCiiJsonSampleController\.v3/);
...
assert.match(sampleController, /ensureManagedStageJsonApi/);
