const APP_LOADER_VERSION = 'input-persistent-root-card-20260629-f';
...
const SHADOW_DIAGNOSTICS_BOOTSTRAP_MODULE_URL = './diagnostics/shadow-diagnostics-runtime-bootstrap.js';
const BUNDLED_ASSETS = window.__3D_MARKUP_BUNDLED_ASSETS__ || {};
const APP_BUNDLE_URL = resolveFromBase(BUNDLED_ASSETS.app || '');

function resolveFromBase(url) { if (!url || !url.startsWith('./')) return url; try { return new URL(url, document.baseURI).href; } catch (_) { return url; } }
...
async function startViewerApp() {
  ...
  import(APP_MODULE_URL).then(() => { markAppLoaded({ bundled: false }); scheduleIdle(loadCanvasToolModeGuard, 50); scheduleIdle(loadManagedStageJsonUiController, 100); scheduleIdle(loadFreshClipController, POST_APP_IDLE_TIMEOUT_MS); scheduleIdle(loadShadowDiagnosticsPanelIfEnabled, POST_APP_IDLE_TIMEOUT_MS + 220); }).catch(handleAppBootError);
}
function loadBundledApp() { setRuntimeStatus('Viewer Loading'); window.__3D_MARKUP_BUNDLED_APP_IMPORT_STARTED__ = true; import(APP_BUNDLE_URL).then(() => { markAppLoaded({ bundled: true }); scheduleIdle(loadCanvasToolModeGuard, 50); scheduleIdle(loadManagedStageJsonUiController, 100); scheduleIdle(loadShadowDiagnosticsPanelIfEnabled, POST_APP_IDLE_TIMEOUT_MS + 220); }).catch(handleAppBootError); }
...
function loadManagedStageJsonUiController() { if (window.__3D_MARKUP_MANAGED_STAGE_JSON_UI_IMPORT_STARTED__) return; window.__3D_MARKUP_MANAGED_STAGE_JSON_UI_IMPORT_STARTED__ = true; Promise.resolve().then(() => import(MANAGED_STAGE_JSON_UI_MODULE_URL)).then(() => import(MANAGED_STAGE_JSON_SAMPLE_MODULE_URL)).then(() => scheduleIdle(loadManagedStageDecorators, 260)).catch((error) => { console.warn('[3DMarkupTool] Managed-stage JSON UI skipped.', error); setRuntimeStatus('Managed-stage UI limited'); }); }
function loadManagedStageDecorators() { if (window.__3D_MARKUP_MANAGED_STAGE_DECORATORS_IMPORT_STARTED__) return; window.__3D_MARKUP_MANAGED_STAGE_DECORATORS_IMPORT_STARTED__ = true; Promise.resolve().then(() => import(MANAGED_STAGE_VISIBLE_FALLBACK_MODULE_URL)).then(() => import(MANAGED_STAGE_COMPONENT_PRIMITIVE_SYMBOLS_MODULE_URL)).then(() => import(MANAGED_STAGE_INPUTXML_CLASSIFICATION_GUARD_MODULE_URL)).then(() => import(MANAGED_STAGE_GEOMETRY_LEDGER_MODULE_URL)).then(() => import(MANAGED_STAGE_SUPPORT_SOURCE_UI_MODULE_URL)).then(() => import(MANAGED_STAGE_SUPPORT_SOURCE_PREVIEW_MODULE_URL)).then(() => import(MANAGED_STAGE_PROFILE_SUPPORT_SOURCE_BRIDGE_MODULE_URL)).then(() => import(MANAGED_STAGE_SUPPORT_PREVIEW_AUTO_APPLY_MODULE_URL)).then(() => import(MANAGED_STAGE_SUPPORT_UI_VISUAL_CLEANUP_MODULE_URL)).then(() => import(MANAGED_STAGE_SUPPORT_DEBUG_LOG_MODULE_URL)).then(() => import(MANAGED_STAGE_SUPPORT_MAPPER_DIAGNOSTICS_UI_MODULE_URL)).then(() => import(MANAGED_STAGE_ISONOTE_WORKFLOW_UI_MODULE_URL)).then(() => import(MANAGED_STAGE_SUPPORT_SETTINGS_POPUP_UI_MODULE_URL)).catch((error) => { console.warn('[3DMarkupTool] Managed-stage decorator modules skipped.', error); }); }
function loadShadowDiagnosticsPanelIfEnabled() { if (window.__3D_MARKUP_SHADOW_DIAGNOSTICS_IMPORT_STARTED__) return; if (!isShadowDiagnosticsMaybeEnabled()) return; window.__3D_MARKUP_SHADOW_DIAGNOSTICS_IMPORT_STARTED__ = true; import(SHADOW_DIAGNOSTICS_BOOTSTRAP_MODULE_URL).then(({ maybeMountShadowDiagnosticsPanel }) => maybeMountShadowDiagnosticsPanel()).catch((error) => { console.warn('[3DMarkupTool] Shadow diagnostics panel skipped.', error); }); }
function isShadowDiagnosticsMaybeEnabled() { try { const queryEnabled = new URLSearchParams(window.location && window.location.search || '').get('shadowDiagnostics') === '1'; const storageEnabled = window.localStorage && window.localStorage.getItem('3dmt.shadowDiagnostics.enabled') === 'true'; return queryEnabled || storageEnabled; } catch (_) { return false; } }
function scheduleAfterFirstPaint(callback) { const run = () => { if (typeof requestIdleCallback === 'function') requestIdleCallback(callback, { timeout: APP_BOOT_IDLE_TIMEOUT_MS }); else setTimeout(callback, 80); }; if (typeof requestAnimationFrame === 'function') requestAnimationFrame(() => requestAnimationFrame(run)); else setTimeout(run, 0); }
function scheduleIdle(callback, timeout) { if (typeof requestIdleCallback === 'function') requestIdleCallback(callback, { timeout }); else setTimeout(callback, Math.min(timeout, 250)); }
function setRuntimeStatus(text) { const el = document.getElementById('runtimeStatus'); if (el) el.textContent = text; }
