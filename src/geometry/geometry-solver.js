...
export function resolvePlantGraphGeometry(graph, bindingAudit, options = {}) {
  ...
  for (const item of items) {
    ...
    if (item.kind === 'component') {
      unresolvedGeometry.push(blockedGeometry(item, binding));
    }
  }
...
export function buildGeometryResolutionAudit(graph, resolvedGeometry, bindingAudit, options = {}) {
  const validation = validateResolvedGeometryModelContract(resolvedGeometry, {
  ...
  const audit = {
    ...
    itemFrameCount: itemFrames.length,
...
function supportPlacement(item, nodesById) {
  ...
  return compact({
    itemId: item.id,
    node,
    position: point(nodePosition || placementPosition || [0, 0, 0]),
    axis: item.axis || '+Y',
    supportFamily: item.supportFamily,
    geometryStatus: 'intentOnly',
    resolver: 'supportPlacementIntent.v1',
    sourceRef: item.sourceRef
  });
}
...
function catalogueComponentFrame(item, binding, nodesById, routeFrameById) {
  ...
  return compact({
    ...
    geometryStatus: 'catalogueFrameResolved',
    resolver: 'catalogueComponentFrame.v1',
    sourceRef: item.sourceRef
  });
}
...
function blockedGeometry(item, binding = {}) {
  return compact({
    itemId: item.id,
    family: binding.family || item.family || item.catalogueRef?.family,
    type: binding.type || item.type || item.catalogueRef?.type,
...
function unit(vector) {
  ...
  return length > 0 ? vector.map((entry) => entry / length) : [0, 0, 0];
}
