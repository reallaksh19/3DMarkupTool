...
assert.match(controller, /data\.view = TOOL_VIEW/, 'controller must create a view-pad button with a data-view marker');
...
assert.match(controller, /event\.stopImmediatePropagation\?\.\(\)/, 'controller must stop app selection/orbit handlers during the marquee drag');
