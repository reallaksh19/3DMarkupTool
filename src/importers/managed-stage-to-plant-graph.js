...
function componentItem(record, a, id, routes, routeByRecordId) { const explicitFamily = value(a, 'CATALOGUE_FAMILY', 'CATALOG_FAMILY', 'FAMILY'); const explicitType = value(a, 'CATALOGUE_TYPE', 'CATALOG_TYPE', 'COMPONENT_TYPE'); const family = explicitFamily || inferredFamily(record); const type = explicitType || value(a, 'DTXR', 'RAW_TYPE', 'CANONICAL_TYPE', 'TYPE') || normType(record); const nps = value(a, 'NPS', 'BORE', 'DIAMETER', 'NOMINAL_SIZE'); const schedule = value(a, 'SCHEDULE', 'SCH', 'WALL_THICK'); const route = routeByRecordId.get(id); const hasCatalogueRef = Boolean(explicitFamily && explicitType); const item = hasCatalogueRef ? { id, kind: 'component', tagged: true, catalogueRef: compact({ catalogue: CATALOGUE.id, family: explicitFamily, type: explicitType, nps, schedule }), placement: componentPlacement(record, a, routes, route), sourceRef: id } : { id, kind: 'component', tagged: false, family, type, resolutionIntent: 'unresolved', route, placement: componentPlacement(record, a, routes, route), sourceRef: id, dimensions: compact({ diameterMm: numberValue(value(a, 'OUTSIDE_DIAMETER_MM', 'DIAMETER_MM', 'DIAMETER')), wallMm: numberValue(value(a, 'WALL_THICKNESS_MM', 'WALL_THICK')) }), bendEvidence: isBendRecord(record) ? bendEvidence(a) : undefined }; return compact(item); }
