...
const FLANGE_DEFERRED_REASON = 'FLANGE_CYLINDER/code8 RVM byte writer bridge not implemented in Phase 11C';
  ...
  if (canCompile) {
    for (const primitive of Array.isArray(primitiveModel?.primitives) ? primitiveModel.primitives : []) {
      ...
      if (primitive.primitiveKind === 'FLANGE_CYLINDER' && Number(primitive.primitiveCode) === 8) {
        deferredExports.push(copyDefined({ sourcePrimitiveId: primitive.primitiveId, sourceItemId: primitive.sourceItemId, family: 'flange', type: primitive.type || 'flange', primitiveKind: 'FLANGE_CYLINDER', primitiveCode: 8, exportStatus: 'deferred', writerReady: false, testByteEligible: false, byteBridge: 'not-implemented-phase-11c', reason: FLANGE_DEFERRED_REASON, resolver: primitive.resolver, catalogueItemId: primitive.catalogueItemId, catalogueRef: primitive.catalogueRef, sourceRef: primitive.sourceRef }));
      }
    }
