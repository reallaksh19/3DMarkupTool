...
try {
  ...
  for (const svg of [topSvg, sideSvg, isoSvg]) {
    assert.match(svg, /<svg[^>]+BM_CII GLB catalogue/i, 'SVG must keep a descriptive accessible label');
    assert.match(svg, /data-component-class="VALVE"/, 'SVG must include valve component cards');
    assert.match(svg, /data-component-class="FLANGE"/, 'SVG must include flange component cards');
    assert.match(svg, /data-role="VALVE_BODY"/, 'SVG must expose valve body role data for visual review');
    assert.match(svg, /data-role="FLANGE_DISC|data-role="FLANGE_PLATE/, 'SVG must expose flange plate/disc role data for visual review');
