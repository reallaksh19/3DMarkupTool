...
export function buildTestArtifactAdapterPlan(exportModels, exportAudit, writerAdapterPlan, writerAdapterAudit, options = {}) {
  const mode = 'testOnly';
  const rvmArtifact = adaptRvmExportModelToTestArtifact(exportModels?.rvmExportModel, exportAudit, writerAdapterPlan, writerAdapterAudit, options);
  const attArtifact = adaptAttExportModelToTestArtifact(exportModels?.attExportModel, exportAudit, writerAdapterPlan, writerAdapterAudit, options);
...
export function buildTestArtifactAdapterAudit(testArtifactPlan, exportModels, writerAdapterPlan, writerAdapterAudit, options = {}) {
  const validation = validateTestArtifactAdapterPlanContract(testArtifactPlan);
  const forbiddenHits = collectTestArtifactForbiddenFieldHits(testArtifactPlan);
  ...
  const audit = {
    schema: TEST_ARTIFACT_AUDIT_SCHEMA,
    graphId: testArtifactPlan?.graphId || options.graphId || '<unknown-graph>',
    mode: testArtifactPlan?.mode || 'testOnly',
    rvmArtifactReady: testArtifactPlan?.rvmArtifact?.artifactReady === true,
    rvmArtifactGenerated: testArtifactPlan?.rvmArtifact?.artifactGenerated === true,
    ...
    attArtifactTextLength: Number(testArtifactPlan?.attArtifact?.textLength || 0),
    attArtifactChecksumPresent: Boolean(testArtifactPlan?.attArtifact?.checksumSha256),
    glbArtifactReady: testArtifactPlan?.glbArtifact?.artifactReady === true,
    glbArtifactGenerated: testArtifactPlan?.glbArtifact?.artifactGenerated === true,
    glbArtifactBlocked: testArtifactPlan?.glbArtifact?.artifactBlocked === true,
    glbArtifactByteLength: Number(testArtifactPlan?.glbArtifact?.byteLength || 0),
    ...
    blockedArtifactItemCount: blockedItems.length,
    deferredArtifactItemCount: deferredItems.length,
